...
var dk =
{
  ...
  splash: function () {
    if (device.iOS && !navigator.standalone)
      return dk.requireFullScreen();
    sound.init();
    dk.addEventListener($('splash'), "touchstart", dk.touchSplash);
    dk.addEventListener($('splash'), "mousedown", dk.touchSplash);
    dk.addEventListener(document, "keydown", dk.doKeyDown);
    dk.addEventListener(window, 'orientationchange', dk.onOrientationChange);
  },
  requireFullScreen: function () {
    $('dialog').innerHTML = 'To install Donkey Kong II, press the share button <br />next to Safari\'s adress bar. From there choose <br />"Add to Home Screen".';
  ...
  generateStateClassNames: function () {
    var cssText = [];
    var els = $("game-and-watch").getElementsByTagName("div");
  ...
  redraw: function () {
    ...
    $("game-and-watch").className = state.join(' ');
  },
